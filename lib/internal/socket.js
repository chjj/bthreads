...
class Socket extends EventEmitter {
  ...
  _init() {
    ...
    if (this._port === this._threads.parentPort)
      EventProxy.listen(this, this._port, 'error', onError);
    else
  ...
  async _handleCall(id, name, args) {
    ...
    switch (items.length) {
      ...
      case 0:
        break;
      default:
        throw new DataCloneError(errors.RESULT);
    }
  ...
  unbind(name, handler) {
    if (blacklist.has(name))
      throw new Error(`Cannot unbind blacklisted event: "${name}".`);
  ...
  async _wait(name, closer) {
    return new Promise((resolve, reject) => {
      let onClose, onError;

      const cleanup = () => {
        this.removeListener(name, onClose);
      ...
      this.on(name, onClose);
      this.on('error', onError);

      if (closer) {
        try {
          closer.call(this._port);
        } catch (e) {
          onError(e);
        }
      }
...
class ThreadBase extends Socket {
  ...
  async close() {
    ...
    return this._wait('exit', this._port.terminate);
  }
...
class Port extends Socket {
  ...
  async close() {
    ...
    return this._wait('close', this._port.close);
  }
...
class PoolBase extends EventEmitter {
  constructor(threads, file, options) {
    ...
    this.size = options.size || getSize(threads.browser);
    this.events = new EventEmitter();
...
function cpus(browser) {
  if (browser) {
    const len = global.navigator
...
function getSize(browser) {
  const len = cpus(browser);
...
function inject(threads) {
  ...
  if (typeof threads.Worker !== 'function'
      ...
      || typeof threads.browser !== 'boolean') {
    throw new TypeError('Invalid worker backend.');
  }

  threads.bufferify = true;
  threads.cores = getCores(threads.browser);
  ...
  threads.Thread = class Thread extends ThreadBase {
    constructor(file, options) {
      ...
      if (options && options.eval && options.dirname) {
  ...
  if (!threads.isMainThread)
    threads.parent = new Port(INTERNAL, threads, threads.parentPort);
}
