...
function write(bw, value, state = new EncodeState()) {
  const type = getType(value);
  ...
  switch (type) {
    ...
    case types.OBJECT: {
      if (state.has(value)) {
        bw.writeU8(types.CIRCULAR);
        bw.writeU32(state.get(value));
        return;
      }
  ...
  switch (type) {
    ...
    case types.OBJECT: {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const value = read(br, parent, stack);
    ...
    case types.MAP: {
      ...
      for (let i = 0; i < count; i++) {
        const key = read(br, parent, stack);
        const value = read(br, parent, stack);
    ...
    case types.FILE_LIST: {
      ...
      for (let i = 0; i < count; i++)
        list.push(read(br, parent, stack));
    ...
    case types.IMAGE_DATA: {
      ...
      const data = read(br, parent, stack);
    ...
    case types.CIRCULAR: {
      ...
      if (index >= stack.length)
        throw new Error(`Could not resolve circular reference: ${index}.`);
...
class EncodeState {
  ...
  add(value) {
    this.seen.set(value, this.index);
    this.index += 1;
  }
