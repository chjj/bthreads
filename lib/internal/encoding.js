...
function getSize(value, seen = new Set()) {
  ...
  switch (type) {
    ...
    case types.FUNCTION: {
      ...
      size += bio.sizeVarString(code, 'utf8');
      size += 4;
...
function read(br) {
  ...
  switch (type) {
    ...
    case types.SYMBOL: {
      return Symbol.for(br.readVarString('utf8'));
    }
    case types.BIGINT: {
    ...
    case types.FUNCTION: {
      const code = br.readVarString('utf8');
      const func = new Function(`return (${code}).apply(this, arguments);`);
