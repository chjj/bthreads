...
function write(bw, value, seen = new Map()) {
  ...
  switch (type) {
    ...
    case types.MESSAGE_PORT: {
      if (value._closed || !value._channel)
        throw new DataCloneError(errors.DETACHED);

      if (!value._dead)
        throw new DataCloneError(errors.NO_PORT);
  ...
  switch (type) {
    case types.REFERENCE: {
      ...
      if (index >= refs.length)
        throw new TransferError(`Could not resolve reference: ${index}.`);
    ...
    case types.MESSAGE_PORT: {
      if (!parent)
        throw new TransferError('Cannot handle message port without parent.');
