...
function onceAdd(ee, events, cb) {
  const onAdd = (event) => {
    if (events.includes(event)) {
      ee.removeListener('newListener', onAdd);
      cb();
    }
  };

  ee.addListener('newListener', onAdd);
}

function lazyBind(self, target, event, handler) {
  onceAdd(self, [event], () => {
