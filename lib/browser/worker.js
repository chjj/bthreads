...
class Worker extends EventEmitter {
  ...
  _init(file, options) {
    if (options.eval)
      file = evalScript('[worker eval]', file);
...
function evalScript(name, body) {
  const file = '/' + name;
  const prefix = require('./bundle.json');

  if (!prefix) {
    // Super-minimal browserify.
    prefix = ''
      + 'self.process = {'
      + '  nextTick: function(f) { setTimeout(f, 1); },'
      + '  title: "browser",'
      + '  browser: true,'
      + '  env: {},'
      + '  argv: [],'
      ...
      + '  binding: function(n) { throw new Error("not supported"); },'
      + '  cwd: function() { return "/"; },'
      + '  chdir: function(n) { throw new Error("not supported"); },'
      + '  umask: function() { return 0; }'
      + '};'
      + 'self.require = function(location) {'
      + '  var err = new Error("Cannot find module \'" + location + "\'");'
      + '  err.code = "MODULE_NOT_FOUND";'
      + '  throw err;'
      + '};'
      + 'self.require.resolve = self.require;'
      + 'self.require.main = null;'
      + 'self.require.extensions = { __proto__: null };'
      + 'self.require.cache = { __proto__: null };';
  }
