...
const {Packet, types, once, format} = common;

/*
 * Constants
 */

const HEADER_URL = 'https://unpkg.com/bthreads-bundle@0.0.0/index.js';
...
class Worker extends EventEmitter {
  ...
  _init(file, options) {
    ...
    once(this, ['message', 'online', 'close', 'exit'], () => {
      this._bind();
    });
  ...
  _handleMessage(event) {
    ...
    switch (pkt.type) {
      ...
      case types.STDOUT: {
        if (!this.stdout)
          throw new Error('Received stdout with no stream available.');

        this.stdout.push(toBuffer(pkt.value));
      ...
      case types.STDERR: {
        if (!this.stderr)
          throw new Error('Received stderr with no stream available.');

        this.stderr.push(toBuffer(pkt.value));
...
class Stdin extends Stream {
  ...
  write(data, enc) {
    ...
    this._parent._write(new Packet(types.STDIN, data));
