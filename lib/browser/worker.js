...
class Worker extends EventEmitter {
  ...
  _init(file, options) {
    ...
    if (options.eval) {
      // Create an object or data URL
      // to the eval header/bundle file.
      // Only works with regular scripts.
      if (options.type === 'module')
        throw new Error('Cannot eval an ES module.');

      url = createWorkerURL(options.header);
      code = file;
      file = null;
    } else if (!isSameOrigin(file)) {
      // Allow cross-origin workers with
      // the same hack we use for evaling.
      // Only works with regular scripts.
      if (scriptType(options, file) !== 'module') {
        url = createWorkerURL(file);
        file = null;
      }
    }

    // Register the worker with bmocha.
...
function createWorkerURL(url) {
  ...
  // but it does add a lexically scoped
  // `__bthreads_importScripts` function.
...
function scriptType(options, file) {
