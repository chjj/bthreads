/*!
 * parser.js - worker parser for bthreads
...
class Parser extends EventEmitter {
  ...
  parseHeader(data) {
    ...
    const port = data.readUInt32LE(1);
    const size = data.readUInt32LE(5);

    return new Header(cmd, port, size);
  }
  ...
  parsePacket(header, data) {
    ...
    pkt.value = encoding.decode(data);
    return pkt;
  }
}
