...
class MessagePort extends MessagePortBase {
  ...
  start() {
    ...
    if (this._closed)
      throw new Error('Message port is closed.');
...
function activate(transferList, parent) {
  ...
  for (const item of transferList) {
    if (item instanceof MessagePort) {
      ...
      if (local._id < MIN_ID || parent._ports.has(local._id))
        throw new Error('Message port ID collision.');
...
function proxy(port1, port2) {
  ...
  // We can cheat here and make the
  // collection function fairly
  // optimal since we know the
  // message will _not_ contain:
  // - DOM objects
  // - Promises
  // - Buffers
  // - SharedArrayBuffers
  ...
  port1.on('message', (msg) => {
    port2.postMessage(msg, collect(msg));
  });

  port2.on('message', (msg) => {
    port1.postMessage(msg, collect(msg));
  });
}

function collect(value, list = []) {
  if (value === null
      || typeof value !== 'object'
