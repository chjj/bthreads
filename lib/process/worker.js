...
const LOADER = `--loader=${path.resolve(__dirname, 'loader.mjs')}`;
...
class Worker extends EventEmitter {
  ...
  _init(file, options) {
    ...
    if (!isBundled())
      ensureArgument(args, `--require=${__dirname}`);
    ...
    if (options.eval) {
      if (options.type === 'module') {
        args.push(LOADER);
        args.push(this._writeModule(file, 'mjs'));
      } else {
        if (isBundled())
      ...
      if (file.startsWith(INLINE_PREFIX)) {
        inlineID = file.substring(INLINE_PREFIX.length);
        file = process.argv[1];
      }
    ...
    const opt = {
      ...
      env: Object.assign(Object.create(null), env, {
        ...
        BTHREADS_WORKER_ROOT: options.dirname
          ? path.resolve(options.dirname)
          : '',
        BTHREADS_WORKER_INLINE: inlineID
      })
    };
