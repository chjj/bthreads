...
class Worker extends EventEmitter {
  ...
  _init(file, options) {
    ...
    if (options.resourceLimits) {
      ...
      if (typeof maxYoungGenerationSizeMb === 'number')
        limits[0] = maxYoungGenerationSizeMb;

      if (typeof maxOldGenerationSizeMb === 'number')
        limits[1] = Math.max(maxOldGenerationSizeMb, 2);

      if (typeof codeRangeSizeMb === 'number')
        limits[2] = codeRangeSizeMb;

      if (limits[0] > 0)
        args.push(`--max-semi-space-size=${limits[0]}`);

      if (limits[1] > 0)
        args.push(`--max-old-space-size=${limits[1]}`);
    ...
    this._parser.on('error', (err) => {
      ...
      if (this._limits) {
        ...
        this._limits = false;
        this._emitError(new WorkerError(errors.OUT_OF_MEMORY));
      } else {
        this._emitError(err);
      }
    });
    ...
    this.resourceLimits = {
      maxYoungGenerationSizeMb: limits[0],
      maxOldGenerationSizeMb: limits[1],
      codeRangeSizeMb: limits[2]
    };
